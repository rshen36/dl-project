...
class MujocoPolicy(Policy):
    def _initialize(self, ob_space, ac_space, ac_bins, ac_noise_std, nonlin_type, hidden_dims, connection_type):
        ...
        with tf.variable_scope(type(self).__name__) as scope:   # ???
            ...
            in_mean = tf.placeholder(tf.float32, ob_space.shape)   # in?
            ...
            # Policy network
            o = tf.placeholder(tf.float32, [None] + list(ob_space.shape))   # what tf is o
    ...
    def _make_net(self, o):
        # Process observation
        if self.connection_type == 'ff':   # 'ff' = fc to fc?
            x = o
        ...
        ac_bin_mode, ac_bin_arg = self.ac_bins.split(':')   # TODO: check that this holds for my envs
        ...
        if ac_bin_mode == 'uniform':
            ...
            a = U.dense(x, adim, 'out', U.normc_initializer(0.01))   # a = set of all possible actions?
        else:
    ...
    @property
    def needs_ref_batch(self):   # why is a reference batch not needed?
        return False
