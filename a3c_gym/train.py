...
def new_cmd(session, name, cmd, mode, logdir, shell):
    if isinstance(cmd, (list, tuple)):
        cmd = " ".join(pipes.quote(str(v)) for v in cmd)
    if mode == 'tmux':
        return name, "tmux send-keys -t {}:{} {} Enter".format(session, name, pipes.quote(cmd))
    elif mode == 'child':
        return name, "{} >{}/{}.{}.out 2>&1 & echo kill $! >>{}/kill.sh".format(cmd, logdir, session, name, logdir)
    elif mode == 'nohup':
        return name, "nohup {} -c {} >{}/{}.{}.out 2>&1 & echo kill $! >>{}/kill.sh".format(shell, pipes.quote(cmd), logdir, session, name, logdir)
...
def create_commands(session, num_workers, env_id, logdir, shell='bash', mode='tmux', visualise=False):
    ...
    cmds = [
        ...
        "echo {} {} > {}/cmd.sh".format(sys.executable, ' '.join([pipes.quote(arg) for arg in sys.argv if arg != '-n']), logdir),
    ]
